...
@Controller_blueprint.record_once
def on_load(state):
    global CARPETA
...
@Controller_blueprint.route('/destroy/<int:Emp_Id>')
def destroy(Emp_Id):
    conn = mysql.connect()
    cursor = conn.cursor()
    ...
    conn.commit()
    return redirect('/Dashboard-Admin/empleados/vista')

@Controller_blueprint.route('/edit/<int:Emp_Id>')
def edit(Emp_Id):
    conn = mysql.connect()
    cursor = conn.cursor()
...
@Controller_blueprint.route('/update', methods=['POST'])
def update():
...
@Controller_blueprint.route('/create')
def create():
    return render_template('/Dashboard-Admin/empleados/create.html')

@Controller_blueprint.route('/store', methods=['POST'])
def storage():
    _Emp_Nombre = request.form['ENombre']
    _Emp_Correo = request.form['ECorreo']
    _Emp_Foto = request.files['EFoto']
